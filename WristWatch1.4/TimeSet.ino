 ...
 byte overlapping = 0;
  ...
  switch (TimeStep){
    ...
    case 1://set hours
      ...
      pixels.clear();
      orientationLights();
      pixels.setPixelColor((TimeHours-1), pixels.Color(0,255,0));
      pixels.show();
    ...
    case 2: // set minutes in 5er Blocks
      ...
      if(upButton == 0 && downButton == 1 && upButtonrelease ==0 && downButtonrelease ==0){ //just down pressed and released
       ...
       if(TimeMinutes>250){TimeMinutes=55;}
      if(TimeMinutes>55 && TimeMinutes<250){TimeMinutes=0;}
            
      pixels.clear();
      orientationLights();
      overlapping = 0;
      if(TimeMinutes<5){if (12!=TimeHours){
        pixels.setPixelColor((TimeHours-1), pixels.Color(0,255,0)); overlapping=0;}else{overlapping=255;} 
        pixels.setPixelColor(11, pixels.Color(255,overlapping,0));
      }else{
        if ((TimeMinutes/5) !=TimeHours){ pixels.setPixelColor((TimeHours-1), pixels.Color(0,255,0)); overlapping=0;}else{overlapping=255;}
         pixels.setPixelColor(((TimeMinutes/5)-1), pixels.Color(255,overlapping,0));
      }
      pixels.show();
    ...
    case 3:  //set minutes precise
      ...
      if(upButton == 0 && downButton == 1 && upButtonrelease ==0 && downButtonrelease ==0){ //just down pressed and released
       ...
       if(TimeMinutes>250){TimeMinutes=59;}
       if(TimeMinutes>59 && TimeMinutes<250){TimeMinutes=0;}
       ...
       if(TimeMinutes<5){
          TimeMinuteLed=12; 
          if(TimeRepetitions==7){TimeRepetitions = TimeMinutes +1; TimeMinutesLedTime= millis(); }
       }else{
       ...
       if(TimeRepetitions==7){TimeRepetitions = TimeMinutes -(TimeMinuteLed * 5 )+1; TimeMinutesLedTime= millis(); }
       overlapping = 0;
       if (TimeMinuteLed != TimeHours){ pixels.setPixelColor((TimeHours-1), pixels.Color(0,255,0)); overlapping=0;}else{overlapping=255;}

  
        pixels.setPixelColor((TimeMinuteLed-1), pixels.Color(255,overlapping,0));
